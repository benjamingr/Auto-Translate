'use strict';
angular.module('tipranksDashboardApp').controller('StocksCtrl', function ($scope, BusinessRules, Stocks, ControlFiltersFactory, userService, $timeout, AnalyticsService, $rootScope) {
    $rootScope.seoTitle = translated('seo-title', 'bar');
    $rootScope.seoDescription = translated('seo-description', 'foo');
    var source = {};
    ...
    $scope.handleCheckboxChange = function (criteria, option) {
        ...
        filterStocks();
        AnalyticsService.report(getStateString(option.val) + '-settings-' + criteria.name.replace(' ', '-') + '-most-recommended', { option: option.name });
    };
    ...
    function toggleAll(options, val) {
        ...
        filterStocks();
        AnalyticsService.report(getStateString(val) + '-settings-sectors-most-recommended', { option: 'allSectors' });
    }
